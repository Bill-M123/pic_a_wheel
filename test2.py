...
def analyze_action(action, action_amount, players, player, dealer):
    ...
    if action == 'fold':
        ...
        dealer.check_count += 1
        if dealer.check_count == 1:
...
while not dealer.betting_complete:
    ...
    if guy.p_nickname == dealer.last_raise:
        ...
        for p in players:
            if guy.p_nickname == p.p_nickname:
                ...
                # print(f"dealer.last_raise==guy.p_nickname {dealer.last_raise==guy.p_nickname}  not dealer.new_bet {not dealer.new_bet}")
                # if ((dealer.last_raise==guy.p_nickname) and ((not dealer.new_bet)and (dealer.check_count!=1))) \
                # or ((dealer.last_raise==guy.p_nickname) and (dealer.check_count>1)):
