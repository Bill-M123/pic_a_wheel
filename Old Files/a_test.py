...
def analyze_action(action, action_amount, players, player, dealer):
    print(player.p_nickname, action, action_amount)
...
actions = ['check', 'bet', 'call', 'bet', 'check']
amounts = [0, 5, 0, 15, 25]
...
for r in range(5):
    ...
    action = actions[r]
    action_amount = amounts[r]

    guy, dealer = analyze_action(action, action_amount, players, guy, dealer)
    print(f"Guy: {guy.p_nickname, guy.bankroll, guy.this_round_per_side}")
    tmp = [x.p_nickname for x in dealer.new_betting_order]
    print(f"dealer.new_betting_order: {tmp}")
    for p in players:
